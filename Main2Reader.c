 ...
 * -------------------------------------------------------------
 */
panda_void displayBuffer(BufferPointer ptr_Buffer);
panda_long getFileSize(panda_strg fname);
panda_intg isNumber(const panda_strg ns);
panda_void startReader(panda_strg, panda_strg, panda_intg, panda_real);
...
panda_intg main2Reader(panda_intg argc, panda_strg* argv) {
	...
	panda_strg program = argv[0];
	panda_strg input = argv[2];
	panda_intg size = 0;
	panda_real factor = 0.0f;
...
************************************************************
*/
panda_void startReader(panda_strg program, panda_strg input, panda_intg size, panda_real factor) {

	BufferPointer bufferp;		/* pointer to Buffer structure */
	panda_intg loadSize = 0;	/* the size of the file loaded in the buffer */
